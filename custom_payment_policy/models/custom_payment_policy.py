...
class AccountMoveInheritPaymentPolicy(models.Model):
    ...
    def _compute_l10n_mx_edi_cfdi_request(self):
        for move in self:
            ...
            elif (move.payment_id and move.payment_id.payment_type == 'inbound' and 'PPD' in move._get_reconciled_invoices().mapped('custom_l10n_mx_edi_payment_policy')):
...
class AccountEdiFormat(models.Model):
    ...
    def _l10n_mx_edi_export_payment_cfdi(self, move):
        ...
        account_payment_id = self.env['account.payment'].search([('move_id', '=', move.id)])
        cfdi_payment_datetime = datetime.combine(fields.Datetime.from_string(move.date), datetime.strptime('12:00:00', '%H:%M:%S').time())
        cfdi_values['cfdi_payment_date'] = account_payment_id.x_studio_fecha_y_hora_de_pago.strftime('%Y-%m-%dT%H:%M:%S')
