...
class CustomReportWizard(models.TransientModel):
    ...
    def print_xlsx(self):
        ...
        query1 = """SELECT DISTINCT account_move.name as invoice_number,
                            ...
                            account_payment_register.payment_date as invoice_payed_date,
        					account_payment_register.amount as payed_amount
        					...
        					left join account_payment_register on account_payment_register.communication=account_move.name """

        # combination of no invoice line and all invoices
        ...
        if self.report_detail == 'no_invoice_line_detail' and self.report_type == 'all_invoices':
            ...
            if self.invoice_initial_date and self.invoice_final_date:
                self.env.cr.execute(
                    query1 + " where account_move.state='posted' and account_move.company_id=%(company_id)s and account_move.date >= %(invoice_initial_date)s and account_move.date <= %(invoice_final_date)s and account_move.move_type='out_invoice'",
                ...
                self.env.cr.execute(
                    query1 + " where account_move.state='posted' and account_move.company_id=%(company_id)s and account_move.date >= %(invoice_initial_date)s and account_move.date <= %(invoice_final_date)s and account_move.move_type='out_invoice'",
                    {"invoice_initial_date": today,
                     "invoice_final_date": self.invoice_final_date,
                     'company_id': self.env.company.id})
                excel_results = self.env.cr.dictfetchall()
            else:
                self.env.cr.execute(
                    query1 + " where account_move.state='posted' and account_move.company_id=%(company_id)s and account_move.move_type='out_invoice'",
                    {'company_id': self.env.company.id})
                excel_results = self.env.cr.dictfetchall()
        ...
        if self.report_detail == 'no_invoice_line_detail' and self.report_type == 'only_paid_invoices':
            ...
            if self.payment_initial_date and self.payment_final_date:
                if self.invoice_initial_date and self.invoice_final_date:
                    self.env.cr.execute(
                        query1 + " where account_move.state='posted' and account_move.company_id=%(company_id)s and account_move.date >= %(invoice_initial_date)s and account_move.date <= %(invoice_final_date)s and account_payment_register.payment_date >= %(payment_initial_date)s and account_payment_register.payment_date <= %(payment_final_date)s and account_move.move_type='out_invoice' and account_move.payment_state in('paid','in_payment')",
                        {"invoice_initial_date": self.invoice_initial_date,
                         "invoice_final_date": self.invoice_final_date,
                    ...
                    self.env.cr.execute(
                        query1 + " where account_move.state='posted' and account_move.company_id=%(company_id)s and account_move.move_type='out_invoice' and account_move.payment_state in('paid','in_payment') and account_payment_register.payment_date >= %(payment_initial_date)s and account_payment_register.payment_date <= %(payment_final_date)s",
                        {'company_id': self.env.company.id,
                         "payment_initial_date": self.payment_initial_date,
                         "payment_final_date": self.payment_final_date
                         })
        ...
        if self.report_detail == 'invoice_line_detail' and self.report_type == 'only_paid_invoices':
            ...
            if self.payment_initial_date and self.payment_final_date:
                if self.invoice_initial_date and self.invoice_final_date:
                    self.env.cr.execute(
                        query1 + " where account_move.state='posted' and account_move.company_id=%(company_id)s and account_move.invoice_date >= %(invoice_initial_date)s and account_move.invoice_date_due <= %(invoice_final_date)s and account_payment_register.payment_date >= %(payment_initial_date)s and account_payment_register.payment_date <= %(payment_final_date)s and account_move.move_type='out_invoice' and account_move.payment_state in('paid','in_payment')",
                        {"invoice_initial_date": self.invoice_initial_date,
                         "invoice_final_date": self.invoice_final_date,
                         "payment_initial_date": self.payment_initial_date,
                         "payment_final_date": self.payment_final_date,
    ...
    def get_xlsx_report(self, data, response):
        ...
        sheet.write('N1', 'Date', date_format)
        ...
        sheet.write('E3', 'Invoice Payed Date', sub_head)
        ...
        sheet.write('K3', 'Payed Amount', sub_head)
        if self.env.ref('base.main_company').currency_id.name != 'MXN':
            ...
            sheet.write('O3', 'Payed Amount MXN', sub_head)

        # data

        sheet.write('O1', data['todate'], date_content)
        ...
        for i in data['excel_result']:
            ...
            sheet.write(row_number, column_number + 3, i['invoice_date_due'],
                        cell_content)
            if i['invoice_payed_date']:
                sheet.write(row_number, column_number + 4,
                            i['invoice_payed_date'],
                            cell_content)
            else:
                sheet.write(row_number, column_number + 4, 'Not Payed',
                            cell_content)
            ...
            sheet.write(row_number, column_number + 9, i['total_amount'],
                        cell_content)
            if i['payed_amount']:
                sheet.write(row_number, column_number + 10, i['payed_amount'],
                            cell_content)
            else:
            ...
            if self.env.ref('base.main_company').currency_id.name != 'MXN':
                ...
                if i['payed_amount']:
                    sheet.write(row_number, column_number + 14,
                                (data['exchange_rate'] * i['payed_amount']),
                                cell_content)
                else:
                    sheet.write(row_number, column_number + 14,
            ...
            if data['line'] == 1:
                ...
                sheet.write(row_number, column_numbers + 5, 'Discount',
                            sub_head2)
                sheet.write(row_number, column_numbers + 6, 'Sub Total',
                            sub_head2)
                sheet.write(row_number, column_numbers + 7, 'Total',
                            sub_head2)
                if self.env.ref('base.main_company').currency_id.name != 'MXN':
                    sheet.write(row_number, column_numbers + 8,
                                'Total Amount MXN',
                                sub_head2)
                else:
                    sheet.write(row_number, column_numbers + 8, ' ',
                ...
                self.env.cr.execute("""SELECT account_move_line.quantity as line_quantity,
                        ...
                        where account_move_line.move_id=%(invoice_id)s""",
                ...
                for j in line_result:
                    ...
                    sheet.write(row_number, column_numbers + 7, j['line_total'],
                                cell_content)
                    if self.env.ref(
